  ...
  return (
    <section ref={containerRef} className="relative h-[180vh]">
      <div className="sticky top-0 flex h-screen items-center justify-center overflow-hidden bg-background">
        {/* Layer paling belakang: gradient halus buat kedalaman, bukan foto stok */}
        <motion.div
          style={{ y: bgY }}
          className="pointer-events-none absolute inset-0 bg-gradient-to-b from-foreground/[0.04] via-transparent to-transparent"
        />
        ...
        <motion.div
          ...
          className="flex select-none flex-col items-center leading-[0.82]"
        >
          {nameParts.map((word) => (
            <span
              key={word}
