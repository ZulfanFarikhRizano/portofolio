...
const Beam = ({
  ...
  // Warna beam dibatasi di rentang hue kuning "yolk" khas situs ini (bukan random
  // rainbow) supaya tetap konsisten sama brand, cuma variasi tipis tiap beam.
  const hue = 45 + Math.floor(Math.random() * 18);
      ...
      style={
        {
          ...
          "--width": `${width}`,
          "--aspect-ratio": `${ar}`,
          "--background": `linear-gradient(hsl(${hue} 95% 65%), transparent)`
...
// Background efek grid 3D dengan "beam" cahaya yang naik dari tiap sisi.
// Diadaptasi dari referensi Osmo — diganti dari package "motion" ke "framer-motion"
// (sudah dipakai di seluruh situs ini) supaya nggak nambah dependency baru.
export const WarpBackground: React.FC<WarpBackgroundProps> = ({
  ...
  beamDelayMin = 0,
  beamDuration = 4,
  gridColor = "hsl(var(--foreground) / 0.08)",
  ...props
}) => {
