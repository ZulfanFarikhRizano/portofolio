...
const SECRET_CLICK_WINDOW_MS = 3500; // Toleransi waktu antar klik disesuaikan
...
export default function CinematicThemeSwitcher() {
  ...
  // Memakai single counter Ref agar stabil dan tidak mudah ter-reset saat re-render
  ...
  const handleToggle = (event: React.MouseEvent<HTMLButtonElement>) => {
    // 1. Tambah hitungan klik
    clickCount.current += 1;

    // 2. Reset timer agar penghitungan klik berlanjut jika masih dalam window waktu
    ...
    // 3. Jika sudah 5x klik -> Eksekusi pindah ke /admin secara instan
    if (clickCount.current >= SECRET_CLICK_COUNT) {
      if (resetTimer.current) clearTimeout(resetTimer.current);
      clickCount.current = 0;
      
      // Prefetch & langsung jalankan router
      router.push("/admin");
      return;
    }

    // 4. Jalankan efek visual & ganti tema untuk klik 1 sampai 4
    generateParticles();
    ...
    if (typeof document.startViewTransition === "function") {
      try {
        ...
        // Cegah error jika view transition mendadak di-abort oleh klik berikutnya
